...
class mysqldb{
  ...
  function mysqldb($dbname,$dbhost='localhost',$dbuser='root',$dbpass='admin1234'){ # most common config ?
    $this->host   = $dbhost;
    $this->user   = $dbuser;
    $this->pass   = $dbpass;
    $this->dbname = $dbname;
  ...
  */
  function dump_to_file($out_file,$droptables=TRUE,$gziped=TRUE){
    ...
    foreach($tables as $table){
      ...
      if($tabledatas = $this->select_to_array($table)){ # si on a des donn閑s ds la table on les mets
        if($gziped)
          gzwrite($fout,"\n# `$table` DATAS\n\n");
        else
          fwrite($fout,"\n# `$table` DATAS\n\n");
